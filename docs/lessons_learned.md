...
| LL-2026-0014 | 2026-06-09 | Prompt-engineering review (reqa-hwe v1.1.0) | Major | The HWE analyzer prompts asked "how should this be written?" (improvement-mode), which produced polished rewrites but missed the gaps an ASPICE assessor would actually flag. Improvement-mode hides defects; falsification-mode exposes them. | Adopt **Falsification-First**: every analysis first lists the conditions under which the artifact is WRONG/INSUFFICIENT (observation → failure scenario → standard link → minimal fix), then proposes wording. Codified in `docs/prompt_engineering_methodology.md` and appended to all four HWE prompts. | `prompts/aspice_hwe1-4_analyzer.md`, `docs/prompt_engineering_methodology.md` | Closed |
| LL-2026-0015 | 2026-06-09 | Prompt-engineering review | Medium | Prompts accepted unbounded context ("give context"), so generic ISO 26262 paste diluted attention onto facts that did not change the specific task. | Adopt **TSR** (Task-Specific Relevance): only context that changes THIS task is admitted (the specific SYS-REQ, failure mode, HARD constraint); generic standard text is discarded. Added as a Context Completeness gate. | `docs/prompt_engineering_methodology.md`, HWE prompts | Closed |
| LL-2026-0016 | 2026-06-09 | Prompt-engineering review | Medium | When a design decision had undeclared constraints, the model returned "multiple approaches are possible" — non-actionable for governance. | Adopt **Constraint-Stacking**: classify constraints HARD vs SOFT before analysis; treat "multiple approaches" as a symptom of undeclared HARD constraints and demand them rather than emitting equal-weight defaults. Reinforces the existing HWE.2 "no equal weights" rule. | `prompts/aspice_hwe2_analyzer.md`, `docs/prompt_engineering_methodology.md` | Closed |
| LL-2026-0017 | 2026-06-09 | Prompt-engineering review | Minor | No structured ship/no-ship gate for a prompt's own output; quality was judged ad hoc. | Add the **6-point Prompt Regression Test** (Role, Context, Falsification, Format, Confidence, Audit); ship at ≥ 5/6. Documented in `docs/prompt_engineering_methodology.md`, enforced as a closing gate in every HWE prompt. | `docs/prompt_engineering_methodology.md`, HWE prompts | Closed |
| LL-2026-0018 | 2026-06-09 | Architecture review | Major | The prompts existed as Markdown but were never executed — the repo was a prompt set + parsers, not a running pipeline; nothing demonstrated reproducible Claude use, and a naive "LLM does everything" design would risk hallucinated metrics. | Build a **Claude-native pipeline** (`src/aspice_validator/`) with a deterministic guardrail: the parser pre-pass (IDs, numeric checks, FMEDA gate) is the audit baseline and is injected into the prompt as established fact; Claude does only the reasoning the guardrail cannot. Documented in `docs/architecture.md`. | `src/aspice_validator/`, `docs/architecture.md`, `pyproject.toml` | Closed |
| LL-2026-0019 | 2026-06-09 | Architecture review | Medium | An evaluator (or assessor) could not reproduce a run without an API key, and a live run spends tokens just to see what is sent. | Add an honest `--dry-run` path (and automatic no-key fallback): the pipeline assembles and emits the full prompt + deterministic pre-pass with zero tokens, and every live artifact carries a provenance header (model id + token usage). Covered by `tests/test_pipeline_dryrun.py`. | `src/aspice_validator/pipeline.py`, `tests/test_pipeline_dryrun.py` | Closed |
